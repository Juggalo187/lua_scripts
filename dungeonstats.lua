local enabled = true
local allkills = false -- If True then any kill will give random stat
local minmoblevel = 1
local elites = false
...
local function updatestat(player, stat, value, apply)

if coreName == "AzerothCore" then
player:HandleStatModifier( stat, 2, value, apply )
elseif coreName == "TrinityCore" then
player:AddFlatStatModifier( stat, 1, value, apply )
end
...
local function Skuly(eventid, delay, repeats, player)
	...
	local CharID, Strength, Agility, Stamina, Intellect, Spirit = Q:GetUInt32(0), Q:GetUInt32(1), Q:GetUInt32(2), Q:GetUInt32(3), Q:GetUInt32(4), Q:GetUInt32(5)
		...
		updatestat(player, 0, Strength, true)
		updatestat(player, 1, Agility, true)
		updatestat(player, 2, Stamina, true)
		updatestat(player, 3, Intellect, true)
		updatestat(player, 4, Spirit, true)
	return false
	else 
...
if PUID ~= nil then
	...
	if Q then
		...
		updatestat(player, statval, r, true )
		end
end
...
local function HandleplayerMailing(player, creature)
	...
	-- local receiver = 0
	-- if coreName == "AzerothCore" then
	-- receiver = GetGUIDLow(GetPlayerByName(player:GetName()):GetGUID());
	-- elseif coreName == "TrinityCore" then
	-- receiver = player:GetGUIDLow()
	-- end
	local receiver = player:GetGUIDLow()
...
if(intid == 9) then
	if isnum then
		if Agility >= (isnum * conversionrate) then
			...
			updatestat(player, Agilityvar, Agility, false)
			updatestat(player, Strengthvar, Strength, false)
			updatestat(player, Agilityvar, Agility - (isnum * conversionrate), true)
			updatestat(player, Strengthvar, Strength + isnum, true)
		else
...
if isnum then
		if Stamina >= (isnum * conversionrate) then
			...
			WorldDBExecute(string.format("UPDATE Dungeon_Stats SET Strength=%i WHERE CharID=%i", Strength + isnum, PUID))
			updatestat(player, Staminavar, Stamina, false)
			updatestat(player, Strengthvar, Strength, false)
			updatestat(player, Staminavar, Stamina - (isnum * conversionrate), true)
			updatestat(player, Strengthvar, Strength + isnum, true)
		else
...
if isnum then
		if Intellect >= (isnum * conversionrate) then
			...
			WorldDBExecute(string.format("UPDATE Dungeon_Stats SET Strength=%i WHERE CharID=%i", Strength + isnum, PUID))
			updatestat(player, Intellectvar, Intellect, false)
			updatestat(player, Strengthvar, Strength, false)
			updatestat(player, Intellectvar, Intellect - (isnum * conversionrate), true)
			updatestat(player, Strengthvar, Strength + isnum, true)
		else
...
if isnum then
		if Spirit >= (isnum * conversionrate) then
			...
			WorldDBExecute(string.format("UPDATE Dungeon_Stats SET Strength=%i WHERE CharID=%i", Strength + isnum, PUID))
			updatestat(player, Spiritvar, Spirit, false)
			updatestat(player, Strengthvar, Strength, false)
			updatestat(player, Spiritvar, Spirit - (isnum * conversionrate), true)
			updatestat(player, Strengthvar, Strength + isnum, true)
		else
...
if isnum then
		if Strength >= (isnum * conversionrate) then
			...
			updatestat(player, Strengthvar, Strength, false)
			updatestat(player, Agilityvar, Agility, false)
			updatestat(player, Strengthvar, Strength - (isnum * conversionrate), true)
			updatestat(player, Agilityvar, Agility + isnum, true)
		else
...
if isnum then
		if Strength >= (isnum * conversionrate) then
			...
			updatestat(player, Staminavar, Stamina, false)
			updatestat(player, Strengthvar, Strength - (isnum * conversionrate), true)
			updatestat(player, Staminavar, Stamina + isnum, true)
		else
...
if isnum then
		if Strength >= (isnum * conversionrate) then
			...
			updatestat(player, Intellectvar, Intellect, false)
			updatestat(player, Strengthvar, Strength - (isnum * conversionrate), true)
			updatestat(player, Intellectvar, Intellect + isnum, true)
		else
...
if isnum then
		if Strength >= (isnum * conversionrate) then
			...
			updatestat(player, Spiritvar, Spirit, false)
			updatestat(player, Strengthvar, Strength - (isnum * conversionrate), true)
			updatestat(player, Spiritvar, Spirit + isnum, true)
		else
